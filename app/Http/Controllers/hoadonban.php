...
class hoadonban extends Controller
{
    ...
    public function hoadonban_view($mahoadon)
    {
    	$i=1;
    	 $data = DB::table('codeloai')->select('nameLoai','code','idLoai')->get();
    	  $data1 =DB::table("cthoadonban")
    	  ->join('hoadonban', 'cthoadonban.mahoadon', 'hoadonban.mahoadon')
        ->join('sanpham','sanpham.id','cthoadonban.masanpham')
        ->select('cthoadonban.*','sanpham.name','hoadonban.maKh','sanpham.image')
    ...
    public function hoadonban_duyet($mahoadon)
    {
        ...
        DB::table('hoadonban')->where('mahoadon',$mahoadon)->update(['xacnhan'=>'Đã duyệt']);
    	  ...
    	  ->join('hoadonban', 'cthoadonban.mahoadon', 'hoadonban.mahoadon')
        ->join('sanpham','sanpham.id','cthoadonban.masanpham')
        ->select('cthoadonban.*','sanpham.name','hoadonban.makh','hoadonban.gia','hoadonban.ngayban','hoadonban.xacnhan')
        ->where('hoadonban.mahoadon',$mahoadon)
        ->get();
