...
class Home extends Controller
{
    ...
    public function add_laydulieuve(Request $request)
    {
        if($request->isMethod('post'))
        {
            ...
            DB::table('sanpham')->insert([
                ['code'=>$maloai,'name'=>$name,'price'=>$price,'image'=>$image1[6],'sale'=>$chietkhau,'introduce'=>$noidung,'time'=>$datebatdau,'masery'=>$sery,'thongso'=>$thongso,'ngayketthuc'=>$dateketthuc]
            ]);
    ...
    public function view(Request $req){
        $data1 = DB::table('sanpham')->select('image','name','price','id')->where('code', $req->id)->paginate(5);
        // $query=DB::table("codeloai");
        // $data=$query->select("*");
      ...
      public function post_cthd_all_add(Request $req)
    {
        ...
        $layCTHD=DB::table('cthd')->select('MaCTHD','id','Tensanpham','soluong','Gia')->get();
         foreach ($layCTHD as $key => $value) {
             ...
             {
                 DB::table('cthd')->insert([
            ['id'=>$id,'Tensanpham'=>$ten,'soluong'=>$soluong,'Gia'=>$gia,'Mancc'=>$mancc,'TenNCC'=>$tenncc]])->where('id',$id);
             }
             else{
                  $sl=$value->soluong+$soluong;
                  DB::table('cthd')->where('id',$id)->update(['soluong' => $sl]);
